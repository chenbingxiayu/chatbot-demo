...
class BusinessCalendar(models.Model):
    ...
    def _validate_input(self, row: int, date_: str, time_: str):
        try:
            datetime.strptime(date_, self.date_format)
            datetime.strptime(time_, self.time_format)
        except ValueError:
    ...
    def update_items(cls, calendar_dates: List[List[str]]):
        ...
        with transaction.atomic():
            for row, (date_, office_hr_end) in enumerate(calendar_dates, 1):
                cls._validate_input(row, date_, office_hr_end)
